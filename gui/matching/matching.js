const F0=20,F1=20000,MA=[20,50,100,200,500,1000,2000,5000,10000,20000],MI=[30,70,300,700,3000,7000];
const S={phase:1,mag:1,wrap:1,sync:1,v0:F0,v1:F1,cursor:null,owner:null,band:null};const G=[];const lg=x=>Math.log(x)/Math.LN10;
const MOTION={
  amp:0,
  phase:0,
  raf:0,
...
function graph(c,b,p,r,k){const x=c.getContext('2d'),isP=k==='phase',ymin=isP?-180:-12,ymax=isP?180:12,MY=isP?[-180,-90,0,90,180]:[-12,-6,0,6,12],mY=isP?[-135,-45,45,135]:[-9,-3,3,9];let drag=0,mode=null,sx=0,sv0=0,sv1=0,bs=null;const val=f=>isP?ph(f):mg(f),enabled=()=>isP?S.phase:S.mag,yp=(v,T,B)=>B-(v-ymin)/(ymax-ymin)*(B-T),dims=()=>({L:isP?30:25,R:c.clientWidth-7,T:8,B:c.clientHeight-23});
function draw(){let d=Math.max(1,devicePixelRatio||1),w=c.clientWidth,h=c.clientHeight;c.width=Math.max(1,Math.round(w*d));c.height=Math.max(1,Math.round(h*d));x.setTransform(d,0,0,d,0,0);x.clearRect(0,0,w,h);let {L,R,T,B}=dims(),y0=yp(0,T,B);x.fillStyle=isP?'#f3f8fb':'#fff8ef';x.fillRect(0,0,w,h);
...
mY.forEach(v=>{let yy=yp(v,T,B);x.strokeStyle='#dbe5ec';x.beginPath();x.moveTo(L,yy+.5);x.lineTo(R,yy+.5);x.stroke()});MY.forEach(v=>{let yy=yp(v,T,B);x.strokeStyle='#b8c8d3';x.beginPath();x.moveTo(L,yy+.5);x.lineTo(R,yy+.5);x.stroke()});MI.filter(f=>f>=S.v0&&f<=S.v1).forEach(f=>{let xx=xf(f,L,R);x.strokeStyle='#e0e8ee';x.beginPath();x.moveTo(xx+.5,T);x.lineTo(xx+.5,B);x.stroke()});MA.filter(f=>f>=S.v0&&f<=S.v1).forEach(f=>{let xx=xf(f,L,R);x.strokeStyle='#bdcbd5';x.beginPath();x.moveTo(xx+.5,T);x.lineTo(xx+.5,B);x.stroke()});x.strokeStyle='#929faa';x.lineWidth=1.15;x.strokeRect(L+.5,T+.5,R-L-1,B-T-1);
x.font='9px Inter,system-ui';x.fillStyle='#56636f';x.textAlign='right';x.textBaseline='middle';MY.forEach(v=>x.fillText(isP?v+'°':v,L-3,yp(v,T,B)));x.textAlign='center';x.textBaseline='top';MA.filter(f=>f>=S.v0&&f<=S.v1).forEach(f=>x.fillText(f>=1000?(f/1000)+'k':f,xf(f,L,R),B+6));x.strokeStyle='#7e8b97';x.lineWidth=1;x.beginPath();x.moveTo(L,y0+.5);x.lineTo(R,y0+.5);x.stroke();
...
if(enabled()){let N=Math.max(700,Math.floor(R-L)*2);if(!isP){x.beginPath();let a,z;for(let i=0;i<=N;i++){let f=S.v0*(S.v1/S.v0)**(i/N),xx=xf(f,L,R),yy=yp(val(f)+displayWobble(f,isP),T,B);if(i===0){x.moveTo(xx,yy);a=xx}else x.lineTo(xx,yy);z=xx}x.lineTo(z,B);x.lineTo(a,B);x.closePath();x.fillStyle='rgba(232,111,23,.16)';x.fill()}x.strokeStyle=isP?'#1686d9':'#e86f17';x.lineWidth=1.55;x.lineJoin='round';x.lineCap='round';if(isP){let pv=null;for(let i=0;i<=N;i++){let f=S.v0*(S.v1/S.v0)**(i/N),v=val(f),dv=v+displayWobble(f,isP),xx=xf(f,L,R),yy=yp(dv,T,B);if(pv===null||Math.abs(v-pv)>300){if(pv!==null)x.stroke();x.beginPath();x.moveTo(xx,yy)}else x.lineTo(xx,yy);pv=v}x.stroke()}else{x.beginPath();for(let i=0;i<=N;i++){let f=S.v0*(S.v1/S.v0)**(i/N),xx=xf(f,L,R),yy=yp(val(f)+displayWobble(f,isP),T,B);i?x.lineTo(xx,yy):x.moveTo(xx,yy)}x.stroke()}}
...
c.addEventListener('pointermove',e=>{let q=c.getBoundingClientRect(),{L,R}=dims(),xx=e.clientX-q.left;if(drag&&mode==='pan'){let span=Math.log(sv1/sv0),shift=-(xx-sx)/(R-L)*span,mn=sv0*Math.exp(shift),mx=sv1*Math.exp(shift),ratio=mx/mn;if(mn<F0){mn=F0;mx=mn*ratio}if(mx>F1){mx=F1;mn=mx/ratio}S.v0=mn;S.v1=mx;all();return}let f=fx(Math.max(L,Math.min(R,xx)),L,R);if(drag&&mode==='band'){S.band=[bs,f];setC(f,k);return}setC(f,k)});c.addEventListener('pointerdown',e=>{let q=c.getBoundingClientRect(),{L,R}=dims(),xx=e.clientX-q.left;drag=1;sx=xx;sv0=S.v0;sv1=S.v1;if(e.shiftKey){mode='pan';c.style.cursor='grabbing'}else if(e.altKey){mode='band';bs=fx(Math.max(L,Math.min(R,xx)),L,R);S.band=[bs,bs]}else mode='probe';c.setPointerCapture(e.pointerId);draw()});c.addEventListener('pointerup',e=>{drag=0;mode=null;c.style.cursor='crosshair';if(c.hasPointerCapture(e.pointerId))c.releasePointerCapture(e.pointerId);all()});c.addEventListener('pointerleave',()=>{if(!drag)clr(k)});c.addEventListener('dblclick',fitAll);c.addEventListener('wheel',e=>{e.preventDefault();let q=c.getBoundingClientRect(),{L,R}=dims(),xx=e.clientX-q.left,a=fx(Math.max(L,Math.min(R,xx)),L,R),z=Math.exp(e.deltaY*.0012),la=Math.log(S.v0),lb=Math.log(S.v1),lp=Math.log(a),mn=Math.exp(lp+(la-lp)*z),mx=Math.exp(lp+(lb-lp)*z);if(mx/mn<1.06)return;let ratio=mx/mn;if(mn<F0){mn=F0;mx=Math.min(F1,mn*ratio)}if(mx>F1){mx=F1;mn=Math.max(F0,mx/ratio)}S.v0=mn;S.v1=mx;all()},{passive:false});new ResizeObserver(draw).observe(c);let api={draw};G.push(api);return api}
graph(phase,pb,pp,pr,'phase');graph(mag,mb,mp,mr,'mag');tp.onchange=e=>{S.phase=e.target.checked;all()};tm.onchange=e=>{S.mag=e.target.checked;all()};tw.onchange=e=>{S.wrap=e.target.checked;all()};ts.onchange=e=>{S.sync=e.target.checked;all()};fit.onclick=fitAll;cb.onclick=()=>{S.band=null;all()};

const matchingPage=document.querySelector('.matching-page');
