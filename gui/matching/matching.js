...
x.font='9px Inter,system-ui';x.fillStyle='#56636f';x.textAlign='left';x.textBaseline='middle';MY.forEach(v=>x.fillText(isP?v+'°':v,L+5,yp(v,T,B)));x.textAlign='center';x.textBaseline='bottom';MA.filter(f=>f>=S.v0&&f<=S.v1).forEach(f=>x.fillText(f>=1000?(f/1000)+'k':f,xf(f,L,R),B-5));x.strokeStyle='#7e8b97';x.lineWidth=1;x.beginPath();x.moveTo(L,y0+.5);x.lineTo(R,y0+.5);x.stroke();
