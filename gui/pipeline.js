...
const WIRE_NODE_SELECTOR='.measurement-node,.xo-filter-node,.mpgd-filter-node';
const WIRE_CLEARANCE=14;
const WIRE_BEND_COST=24;
const WIRE_PORT_DIRECTION_COST=34;
const WIRE_CORNER_RADIUS=6;

function wireNodeFor(element){
  return element?.closest?.(WIRE_NODE_SELECTOR)||null;
}

function pointInsideWireRect(point,rect){
  const e=.01;
  return point.x>rect.left+e&&point.x<rect.right-e&&point.y>rect.top+e&&point.y<rect.bottom-e;
}

function collectWireObstacles(start,end,sourceElement=null,targetElement=null){
  const canvasRect=nodeCanvas.getBoundingClientRect();
  const sourceNode=wireNodeFor(sourceElement);
  const targetNode=wireNodeFor(targetElement);
  const obstacles=[];
...
function wireSegmentClear(a,b,obstacles){
  const e=.01;
  if(Math.abs(a.x-b.x)<e){
    ...
    for(const rect of obstacles){
      if(x>rect.left+e&&x<rect.right-e&&high>rect.top+e&&low<rect.bottom-e) return false;
    }
    return true;
  }
  if(Math.abs(a.y-b.y)<e){
    ...
    for(const rect of obstacles){
      if(y>rect.top+e&&y<rect.bottom-e&&high>rect.left+e&&low<rect.right-e) return false;
    }
    return true;
  }
  return false;
...
function uniqueWireCoordinates(values){
  const sorted=values.filter(Number.isFinite).sort((a,b)=>a-b);
  const result=[];
  for(const value of sorted){
    if(!result.length||Math.abs(value-result[result.length-1])>.25) result.push(value);
  }
  return result;
}

function simplifyWirePoints(points){
  const clean=[];
  ...
  for(let i=1;i<clean.length-1;i++){
    ...
    const b=clean[i];
    const c=clean[i+1];
    const sameX=Math.abs(a.x-b.x)<.01&&Math.abs(b.x-c.x)<.01;
    const sameY=Math.abs(a.y-b.y)<.01&&Math.abs(b.y-c.y)<.01;
    if(!sameX&&!sameY) result.push(b);
  }
  result.push(clean[clean.length-1]);
  return result;
}

function wirePathFromPoints(points){
  const clean=simplifyWirePoints(points);
  ...
  for(let i=1;i<clean.length-1;i++){
    ...
    const radius=Math.min(WIRE_CORNER_RADIUS,inLen/2,outLen/2);
    if(!(radius>.1)){
    ...
    const before={
      x:corner.x-inDx/inLen*radius,
      y:corner.y-inDy/inLen*radius
    };
    const after={
      x:corner.x+outDx/outLen*radius,
      y:corner.y+outDy/outLen*radius
    };
...
function routeOrthogonalWire(start,end,obstacles){
  if(!obstacles.length){
    const mid=(start.x+end.x)/2;
    return wirePathFromPoints([start,{x:mid,y:start.y},{x:mid,y:end.y},end]);
  }
  ...
  const minX=Math.min(start.x,end.x,...obstacles.map(rect=>rect.left))-32;
  const maxX=Math.max(start.x,end.x,...obstacles.map(rect=>rect.right))+32;
  const minY=Math.min(start.y,end.y,...obstacles.map(rect=>rect.top))-32;
  const maxY=Math.max(start.y,end.y,...obstacles.map(rect=>rect.bottom))+32;
  xs.push(minX,maxX);
  ys.push(minY,maxY);

  const gridX=uniqueWireCoordinates(xs);
  const gridY=uniqueWireCoordinates(ys);
  const width=gridX.length;
  const height=gridY.length;
  const valid=Array.from({length:height},()=>Array(width).fill(true));

  for(let yi=0;yi<height;yi++){
    for(let xi=0;xi<width;xi++){
      const point={x:gridX[xi],y:gridY[yi]};
      if(obstacles.some(rect=>pointInsideWireRect(point,rect))) valid[yi][xi]=false;
    }
  }

  const nearestIndex=(values,value)=>{
    let best=0;
  ...
  const sx=nearestIndex(gridX,start.x);
  const sy=nearestIndex(gridY,start.y);
  const ex=nearestIndex(gridX,end.x);
  const ey=nearestIndex(gridY,end.y);
  valid[sy][sx]=true;
  valid[ey][ex]=true;

  const keyFor=(xi,yi,dir)=>xi+','+yi+','+dir;
  const heap=[];
  const heapPush=item=>{
    heap.push(item);
    let i=heap.length-1;
  ...
  const startKey=keyFor(sx,sy,'n');
  gScore.set(startKey,0);
  heapPush({xi:sx,yi:sy,dir:'n',g:0,f:Math.abs(start.x-end.x)+Math.abs(start.y-end.y),key:startKey});
  ...
  while(heap.length){
    ...
    if(current.xi===ex&&current.yi===ey){
      endState=current;
      break;
    }

    const a={x:gridX[current.xi],y:gridY[current.yi]};
    for(const step of directions){
      const xi=current.xi+step.dx;
      const yi=current.yi+step.dy;
      if(xi<0||yi<0||xi>=width||yi>=height||!valid[yi][xi]) continue;
      const b={x:gridX[xi],y:gridY[yi]};
      if(!wireSegmentClear(a,b,obstacles)) continue;

      const distance=Math.abs(b.x-a.x)+Math.abs(b.y-a.y);
      const bend=current.dir!=='n'&&current.dir!==step.dir?WIRE_BEND_COST:0;
      const sourceDirection=current.dir==='n'&&step.dir==='v'?WIRE_PORT_DIRECTION_COST:0;
      const targetDirection=xi===ex&&yi===ey&&step.dir==='v'?WIRE_PORT_DIRECTION_COST:0;
      const nextG=current.g+distance+bend+sourceDirection+targetDirection;
      const nextKey=keyFor(xi,yi,step.dir);
      if(nextG>=(gScore.get(nextKey)??Infinity)) continue;

      gScore.set(nextKey,nextG);
      previous.set(nextKey,current.key);
      const heuristic=Math.abs(b.x-end.x)+Math.abs(b.y-end.y);
      heapPush({xi,yi,dir:step.dir,g:nextG,f:nextG+heuristic,key:nextKey});
    }
  }

  if(!endState){
    // Fail closed visually: do not draw a route through an obstacle.
    return '';
  }

  const points=[];
  let stateKey=endState.key;
  while(stateKey){
    const [xi,yi]=stateKey.split(',').map(Number);
    points.push({x:gridX[xi],y:gridY[yi]});
    stateKey=previous.get(stateKey)||null;
  }
  points.reverse();
  points[0]=start;
  points[points.length-1]=end;
  return wirePathFromPoints(points);
}

function routeWire(start,end,options={}){
  if(!start||!end) return '';
  const obstacles=collectWireObstacles(
    start,
    end,
    options.sourceElement||null,
    options.targetElement||null
  );
  return routeOrthogonalWire(start,end,obstacles);
}

let wireObstacleFrame=0;
new MutationObserver(mutations=>{
  const touchesNode=mutations.some(mutation=>{
    if(mutation.type==='attributes'){
      return mutation.target?.matches?.(WIRE_NODE_SELECTOR)===true;
    }
    for(const node of [...mutation.addedNodes,...mutation.removedNodes]){
      if(node?.nodeType!==1) continue;
      if(node.matches?.(WIRE_NODE_SELECTOR)||node.querySelector?.(WIRE_NODE_SELECTOR)) return true;
    }
    return false;
  });
  if(!touchesNode) return;

  if(wireObstacleFrame) cancelAnimationFrame(wireObstacleFrame);
  wireObstacleFrame=requestAnimationFrame(()=>{
    wireObstacleFrame=0;
...
function startWire(event,source,handle){
  ...
  const markEligible=()=>{
    ...
    for(const candidate of eligibleRegisteredInputs(source)){
  ...
  const move=moveEvent=>{
    ...
    const magnet=nearestRegisteredInput(moveEvent.clientX,moveEvent.clientY,source);
  ...
  const end=endEvent=>{
    ...
    const magnet=nearestRegisteredInput(endEvent.clientX,endEvent.clientY,source);
