...
const TOOL_ID="nga-editor";
const rbj=window.RaptorEqGeometryRBJ;
if(!rbj) throw new Error('eq-geometry-rbj.js must load before nga-editor-eq-geometry.js');

/*
 * NGA Editor EQ geometry state.
...
function requireContext(body=null){
  ...
  if(!node||!key) throw new Error('NGA Editor EQ geometry requires a connected Pipeline measurement');
  const sampleRateHz=sampleRateOf(node);
  if(!sampleRateHz) throw new Error('NGA Editor EQ geometry requires Pipeline sample-rate metadata');
...
function normalizeForStore(operation,sampleRateHz,id=null){
  ...
  return {
    id:String(normalized.id||("nga"+'-eq-'+sequence++)),
    type:'peaking',
...
function deriveViews(body,baseViews,sampleRateHz=null){
  ...
  if(!Number.isFinite(fs)||fs<=0) throw new Error('NGA Editor EQ geometry has no authoritative sample rate');
...
window.NgaEditorEqGeometry=Object.freeze({
  toolId:TOOL_ID,
  model:rbj.model,
  contextKey,
  getOperations,
