...
assert.match(pipelineCss,/\.measurement-node\{[^}]*position:relative;[^}]*width:100%;min-width:0;max-width:none/);
assert.match(pipelineCss,/\.pipeline-source-stack\{[^}]*width:min\(248px,calc\(100vw - 48px\)\);[^}]*display:flex;flex-direction:column/);
assert.match(pipelineCss,/\.bank-file-node\{[^}]*position:relative;[^}]*width:100%;height:194px;[^}]*grid-template-rows:35px 126px 31px/);
...
assert.match(indexHtml,/pipeline\.css\?v=source-stack-dock-20260916-1/);
...
assert.match(indexHtml,/pipeline-interactions\.css\?v=source-stack-dock-20260916-1/);
assert.match(indexHtml,/pipeline\.js\?v=source-stack-dock-20260916-1/);
assert.match(indexHtml,/pipeline-workspace-theme\.js\?v=forest-aurora-20260915-1/);
assert.match(indexHtml,/forest-aurora\.css\?v=source-stack-dock-20260916-1/);
...
assert.match(indexHtml,/id="pipelineSourceStack"/);
assert.match(indexHtml,/id="sourceStackToggle"[^>]*aria-expanded="true"/);
...
assert.match(indexHtml,/bank-file-source\.js\?v=source-stack-dock-20260916-1/);
assert.match(indexHtml,/bank-file\.js\?v=source-stack-dock-20260916-1/);
...
assert.match(bankSourceJs,/sampleRateHz:48000,fftSize:32768,maxFrequencyHz:24000,color:'#F6C85F'/);
assert.match(bankSourceJs,/sampleRateHz:96000,fftSize:65536,maxFrequencyHz:48000,color:'#36CFC9'/);
assert.match(bankSourceJs,/sampleRateHz:192000,fftSize:131072,maxFrequencyHz:96000,color:'#FF2D95'/);
assert.match(bankFileJs,/api\.registerMeasurementSource\?\.\(entry\)/);
assert.match(bankFileJs,/api\.openMeasurementPreview\?\.\(preview,entry\)/);
assert.doesNotMatch(bankFileJs,/bank-file-swatch/);
assert.match(pipelineJs,/#FF2D95','#8BD600','#563CFF'/);
assert.match(pipelineJs,/bankFile:\{position:null,collapsed:false\}/);
assert.match(pipelineJs,/function positionSourceStack\(x,y\)/);
assert.match(pipelineJs,/function applySourceStackCollapsed\(collapsed,/);
